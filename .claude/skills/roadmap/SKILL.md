...
| **Turned or mirrored balls fall back** (fix parked on branch `wip/turned-ball-sections`; audit probe `zz_pose_audit` in the session scratchpad) | Every boolean tried on a ball whose axis leaves world z falls back to a mesh (a cap, a box corner, a rod, turned or mirrored): a primitive hemisphere's boundary v range, empty upright, is a 1e-16 sliver on a turned ball and `restrict_curves_to_faces` clips every section away. The parked fix (`face_v_range` treats a sliver as empty) makes the cap, rod and corner exact but exposes a chord hemisphere split by two closed latitude sections: the ball less the slab `1 < z < 2` comes back open when turned (the Cut gate tolerates free edges) and missing its lower piece when mirrored (valid, 8.378 against 92.153). The branch also holds two guards those turned cases needed: the collar arrangement (`split_noseam_by_arrangement`) keeps the region inside every arc chain, the whole answer only with three or more chains around the pole (a turned half-space Intersect came back 155.63 against 42.54 with one), and a plane face's arc filter widens its band only by short rim chords' sagitta (a turned box face kept an arc 0.4 outside it through the 4.4 sagitta of its 10-unit side). Next: the two-latitude split, and whether the Cut gate should accept free edges |
| **Pose audit: other primitives** (probe `zz_pose_audit`, 5 primitives x 4 tools x Cut/Intersect x upright/turned/mirrored) | Exact results whose volume moves with the pose: a turned cylinder's box-corner Cut (2e-6), a turned frustum (2e-5 for a half-space or a corner), a mirrored cone's half-space Intersect (9e-6). Fallbacks only when mirrored: a cylinder's box corner, a cone's rod Cut. Fallbacks in every pose: a pointed cone's box-corner and rod Intersects, a frustum's rod, a torus's rod. The slab Cut that leaves two pieces is exact but invalid for the cylinder, cone and frustum in every pose |
...
| **STEP loops on reversed faces** | brepkit keeps a reversed face's wire about the surface's outward normal (the boolean builders flip the flag and keep the wire; the mesher, `transform_solid` and the volume flux read it that way). The STEP reader keeps a `.F.` face's loops as written and the writer writes them as stored, so brepkit round-trips, but by ISO 10303-42 those loops wind about the face normal: a single-circle cap on a reversed face from another system reads as its complement. Next: reverse `.F.` loops on read and on write together, with a fixture from another system |
| **Ray-cast point classifiers flip a reversed face's side** (`classify_point` in `crates/operations/src/classify.rs` near line 470 and `crates/check/src/classify/boundary.rs` near line 288) | Both negate a reversed face's boundary-loop normal before keeping a ray hit on the face's side of the loop, but the wire already runs about the surface's outward normal, so the flip looks on the wrong side. The box `[-5, 5]³` less a ball of radius 2 at `(0, 0, 5.5)`: (0, 0, 4) and (1.9, 0, 4.9) in the dimple read Inside in both, and (0, 0, 3) below it reads Outside in the operations one. The boolean containment checks call it. Next: drop the flip in both and audit bores and cavities |
| **Shelled cylinder and cone cups are invalid** | `shell(make_cylinder(5, 10), 1, [top])` and `shell(make_cone(5, 2, 10), 1, [top])` each leave 32 shared edges with one sense (the cup measures 344.45 against 333.01), and a hollowed `make_torus(6, 2)` fails with "solid assembly produced no faces". Their full-circle inner walls reach `assemble_solid_mixed`'s generic arm with the reversed vertex list and the flag, the pattern the sphere arm drops (the ball's inner wall keeps the outer face's winding and the flag turns it) |
| **Second booleans on a ball's octant** (pin `box_octant_feeds_a_second_boolean` in `crates/operations/tests/sphere_box_corner.rs` tolerates the fallbacks) | The octant from the box shortcut less a box corner at `(1, 1, 1)` falls back to a mesh, and less a rod along z through `(1, 1)` above the equator too (below it is exact). The same octant built by the GFA (box and ball turned 0.3 about z, so the shortcut does not fire) less that corner comes back unchanged and invalid: silently wrong, the next dig |
