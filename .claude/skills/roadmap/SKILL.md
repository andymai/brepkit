...
| **The engine's ray cast reads a ball's hole cut by a column by its flat polygon** (`make_sphere(3, 32)` less `make_box(2w, 2w, 10)` at `(-w, -w, -1)`) | `classify_ray_cast` misreads 855 of 4721 points of a 17-step grid over the ball at `w = 0.1` and 624 at `w = 1.8`, on main as well: the upper hemisphere's hole of four wall arcs also admits the wall circles' runs below the equator, which its own outer loop already excludes, so `sphere_face_loops` declines it and the face takes the flat polygon. A candidate: accept an uncovered admitted run that another loop of the same face rejects, once it is shown that no loop's region can sit inside that run |
