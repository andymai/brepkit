...
| **Assembly cluster fuses degrade to mesh** (`assemblyGenerator.scenario.test.ts`: "arch at the base center" and "tilted block and cradle" throw `cluster fuse degraded to mesh fallback`; operands captured with the `captureScoopOps.test.ts` wrapper pattern, replayed with `replay_pair FUSE_ALL=1 FUSE_MEMBERS=<third member>`) | Two roots. ARCH: the rod (r=4 along X) passes through each post with its top exactly TANGENT to the post's top face (centre z=46, top z=50) and its end cap coincident with the post's outer face; the pairwise post x rod fuse is ACCEPTED closed by edge id with the rod outside the post dropped (vol 4701.8 vs ~7764) and 577 non-manifold mesh edges, and only the three-way cluster validation refuses it. Native repro (a 6x15x50 box plus a rotated `make_cylinder` rod, 361 non-manifold mesh edges) parked in the session scratchpad; `plane_internal_line_loops` drops the tangent line's endpoints as "not strictly interior (dist 0)". TILTED: block x cradle (two tilted parts overlapping 5 mm in x) fails GFA ("open growth shell with 18 faces would be dropped") and the mesh fallback is non-manifold; plate x cradle is accepted closed by id with 100 open mesh edges and a volume 2037 mm³ below the raw result. TUBE: the test passes since the plate x tube nesting closed, but its mouth chamfer (`chamfer(tube, both rims at z=60, 1/3)`) fails cleanly and the tool keeps the sharp mouth: the planar bevel throws `cannot normalize zero vector` (a closed circle's single vertex gives a zero direction) and the blend engine's chamfer returns 8 faces open by edge id (286 open mesh edges); native probes on a `make_cylinder` tube minus a coaxial counterbore, parked in the session scratchpad. Next: the arch tangency, a stable primitive repro |
