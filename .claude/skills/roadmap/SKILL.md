...
| **A cone through a ball off its axis takes minutes, then a mesh** (probe `zz_conesph` in the session scratchpad) | `make_sphere(3, 32)` less `make_cone(1.2, 0.4, 10)` moved to `(1, 0.5, -5)` (axis along `z`, off the ball's) takes 770 s on main and falls back to a 696-face mesh; the Intersect runs past 730 s. Laid along `x` through the ball's middle or tilted, each Cut and Intersect takes 262 to 316 s on main and falls back too. A perf-bar defect, undug: profile phase FF on the Cut first |
