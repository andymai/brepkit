...
| **Faceted socket lofts → loft fix READY on branch `fix/loft-recognize-sketch-arcs`, BLOCKED on un-masked z=5 rim tessellation cracks** | operations/loft (fixed) + tessellate (open) | The old "halfSockets zero-triangle" framing was a MISREAD: the scenario runner logs `triangleCount: 0` for ANY failed case; the three snapshot failures are benign kernel-specific count pins (brepkit 1964/2544/3356 vs reference 2796/5176/8508, all BELOW). The real gap underneath: every gridfinity socket loft came out ALL-PLANE (~1.2–2.5% bin volume deficit, entirely in the z0–5 feet — z-histogram localization). Two loft bails fixed on the branch: NURBS profile edges (recognize→analytic) and downward-stacked CCW sketches (reverse profiles; arc reversal must NEGATE the circle normal or the complementary span is traced). Kernel suites all green; default 1×1/2×2 bins reach EXACT volume parity (13527.2 vs 13527.3). DO NOT MERGE until the un-masked defect is fixed — NOT a tessellation bug: the fuse emits a WIRE-ORIENTATION inconsistency. Diagnosed on op-017 of the 2×2-halfSockets chain (fuse of body × 16-analytic-socket assembly): the kept z=5 cavity-ceiling plane (A's bottom over the socket void, correctly DOWN-facing) and the kept z∈[4.75,5] r=4 corner-cylinder band (B's cavity wall, SPLIT and rebuilt) traverse their shared rim edge in the SAME wire direction (`oe_fwd=true` on both) — orientation-blind free/over passes, but both meshers faithfully emit same-direction half-edges → 314 unpaired mesh edges + inverted-region volume loss (−13%). B alone is consistent and stores its cavity cylinders rev=false (orientation carried by WIRE DIRECTION, not the flag); the GFA rebuild of the split band flips/loses that traversal convention. `1×1 halfSockets` nm=16 is the same family. Repro: captures4/hs2x2 op 017 + probe-branch dumps (DUMP_EDGE_FACES=endpoints prints owners+fwd+rev; DUMP_EDGE_CHAIN compares both faces' welded rim chains; DUMP_BND_NEAR). Fix candidates: preserve source wire traversal in the cylinder-band split rebuild, or add per-edge flip-propagation orientation consistency in `builder_solid` (only whole-shell heuristics exist today). |
