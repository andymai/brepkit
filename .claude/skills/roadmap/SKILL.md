...
|---|---|---|
| **`cone ∪ box` — the ONLY remaining primitive-boolean mesh fallback** (ready-repro `boolean::tests::cone_union_box_should_be_analytic`, `#[ignore]`; census row `cone ∪ box`) | algo/GFA | Root-mapped 2026-07-24. `make_cone(6,2,12)` is r=4 at z=6 where the 8x8 box's bottom sits, so the section circle is INSCRIBED in the box square and TANGENT to all four walls (the recurring tangential-contact class). Raw GFA nearly succeeds: F=10, 1 cone + 9 planes, free=0, rejected only for 4 non-manifold edges. The BOX side is CORRECT — the pinched square-minus-inscribed-circle tiles into 4 curvilinear triangles and each wall splits at its tangency point. The CONE side loses the z=0 base rim: both cone plane faces vanish (z=12 top disc correctly absorbed, but the z=0 base disc r=6 is nowhere near the box and must survive), and the cone lateral's single inner wire repeats its OWN outer wire (the same four z=6 arc edge ids) instead of the base rim — hence each arc used 3x. One missing piece, two symptoms: the remainder is closed by duplicating the wrong rim. TANGENCY IS THE TRIGGER (sweep `diag_cone_box_tangency_sweep`): two independently chosen tangent configs (d=8/zb=6 and d=6/zb=9) fail IDENTICALLY (F=10, z0 disc dropped, 4 nm), while the same fuse with the section circle STRICTLY INSIDE the box is perfectly clean (F=8, validate ok) — so this is not about cones or discs in general. DISCOVERED en route, separate defect: a section circle CROSSING the box walls (d=6/zb=6) gives 1 FREE edge — a third case, unmapped. First probe: why the z=0 rim/disc is dropped (classification vs. never emitted). |
