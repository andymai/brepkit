...
| **`arc_segment_crossings` windows a boundary arc by its shorter-arc midpoint** (`crates/algo/src/builder/fill_images_faces.rs`, the `a_mid` from `evaluate_edge_at_t`) | Found by the review of the native-arc same-domain sampling fix: for a major boundary arc (a rim remainder above 180 degrees) the window is the minor complement, so the labelled major-arc branch never fires and crossings on the true arc are discarded. Same one-line fix as `compute_edge_set_quantized` (`domain_with_endpoints` midpoint); unit-test module `closed_rim_chord_crossing_keeps_seam_angle_hit` takes a 276 degree case. No live repro yet |
