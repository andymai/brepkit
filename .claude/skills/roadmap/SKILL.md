...
|---|---|
| **Tool geometric parity (#1517)** | MEASURED 2026-08-10 on released 3.2.22, stock pins, same-day same-catalog control. Generator suite 272 files / 2693 tests: **3.2.18 154 failed, 3.2.22 144 failed**; excluding stale per-kernel snapshots that is **153 -> 130 real**. The issue's old "137 failed / 2550 passed" DOES NOT REPRODUCE (3.2.18 measures 154 today) — the catalog grew, so only a same-day control is trustworthy. Head-to-head matrix: perf **0.63x** aggregate, faster on 24/26, **all 26 closed with 0 non-manifold** vs the reference's 5. 4x4-EVERYTHING ROW CLOSED 2026-08-13 (the heaviest timeout): its final base fuse (123-face body with a center insert pocket at the FOUR-FOOT junction x 544-face 16-foot base) burned 72.7s wasm in a dirty mesh fallback (over=36); now exact in ~100ms. Three roots: the vertex-coincidence PROMOTION path spliced hole-split sections without the full weave, so the pocket-mouth cells traced as disconnected islands emitted as phantom material; the containing plate piece then lost its inner wire (islands are unreachable from the cell walk) — islands are now discarded AND handed to hole matching (oriented to the ORIGINAL wire winding), gated to the promotion path ONLY (the real weave creates in-hole regions intentionally — tilted-divider cap; the un-woven path spans openings legitimately — dovetail holecut); and the SD geometric group (one plate, sixteen coincident foot tops) demoted every same-rank member to a within-rank duplicate — members not covered by the opposite representative (the pocket bites, the only caps under the open mouth) are now exempt. Fixture `crates/io/tests/gridbin4x4_feet_fuse_inmem.rs` (no-fallback + volume + no NEW orientation issues — the captured body arrives with 34 same-direction divider edges of its own, far from the interface). Remaining 144 by class: 34 open-shell, **19 compound-capability (#1537 — 17 of them are the tool's stale `brepjs` pin at 18.124.2, fixed in brepjs 18.124.7; only the 2 compound-base `fuse` calls are a live gap, and it is brepjs-side)**, 15 timeout, 14 stale snapshot, 12 non-manifold, 2 reentrancy. Harness `kernelParityMatrix.test.ts` + `scripts/compare-kernel-parity.ts` |
| **#1538 tail: the coplanar-interface family, DECOMPOSED** | Dug 2026-08-12 with synthetic clean-input probes (`interface_fuse_probe.rs`): the family is WINDING emitters, not one fuse defect. THREE ROOTS FIXED (see Closed: "Closed-edge winding direction"): internal-loops frame-area inversion on down-facing planes, merge never flipping closed edges, CB rebuild degenerating to forward=true on closed rims. Rect and circle through-hole chains (cut + interface fuse) now strictly valid end-to-end. Coincident-cap pocket rim ALSO FIXED (2026-08-12 follow-up): the quarter-point direction test was frame-dependent — a closed circle's `domain_with_endpoints` anchors at the curve's OWN reference direction, so two opposing circles can compare as same-direction. Replaced with tangent comparison at the shared point (`closed_curves_same_direction`, fill_images_faces); all three synthetic modes (rect/circle/pocket) strictly valid end-to-end, pocket pin un-ignored. Deepcutout line residue ALSO CLOSED (2026-08-12, see Closed: "Free-loop cap synthesis double-covered"): the whole chain (cut + socket fuse) is exact and strictly valid; all deepcutout pins active. Circleinsert WINDING root ALSO CLOSED (2026-08-12, see Closed: "Extrude emitted mirrored wires"): the "stale operand" framing was REFUTED — a same-day re-capture on verified 3.2.28 was byte-identical (sha256) to the 3.2.24 capture; the damage was minted live by `extrude` on the tool's CW-wound profile. REMAINING: (a) circleinsert SOCKET FUSE with BOTH operands validation-clean — native repro `cargo run --release -p brepkit-io --example circleinsert_chain` (FREE_EDGES=1 dumps owners), ignored pin `circleinsert_socket_fuse_is_strictly_valid`; was 84 free + 4 over pre-#1559, now 56 free + 0 over on 3.2.30. MAPPED 2026-08-12, TWICE — the first SD framing was WRONG, do not resume it: the sockets are four BASEPLATE-PROFILE blocks (z∈[0,5], cavity mouth ring 1.39 in from the foot edge, 45° chamfer cone down to 39.60/2.40 walls) fused INTO the bin floor for mini-bin docking; they have NO 40.55 walls and NO 2.55 corner cylinders, so the hypothesized coincident-wall/corner SD pairs DO NOT EXIST (detect_same_domain's 9 pairs are likely all there is; only the 3-of-4 bottom-plane `planar_faces_overlap` misses merit one look). LIVE HYPOTHESIS: the bin wall plane's keep/drop fencing is INCOMPLETE — builder-stage SUBWIRE Id(249) spans both the exposed wall region and the z 1.2..3.55 strips embedded in foot material, connected through the inter-feet gap; SECOND FRAMING ALSO RETRACTED same-day (the foot flares OUTWARD going up, so the wall's drop band is the middle strip z 3.55..5 with an apparently CLOSED fence, and SUBWIRE Id(249) is plausibly a correct keep region): the freed faces are NOT the 40.55 walls — they are slab-top gap-channel pieces (y/x=±2.4 lines at z=1.2, ending at the pocket rim crossing x=±9.708), pocket-wall cylinder pieces, socket corner cones, and bin corner cylinders. MECHANISM FOUND 2026-08-12 (single-edge trace + stage counts): the splitter and classification are fine (the channel-wall partners exist, split correctly, classify Outside, get selected — 227 faces), but `remove_doubled_faces` (builder_solid Step 0b2) drops 11 of them in 4 groups: the corner cone∩cylinder LENS pieces. After merge_duplicate_edges, the cylinder-side and cone-side lens faces reference the IDENTICAL two edge entities, and the pass groups by edge-ID multiset alone — its "identical boundary ⇒ zero volume ⇒ cancel" argument silently assumes SAME SURFACE; a two-edge lens between two different quadrics is the counterexample (real volume, both faces legitimate boundary). The avoidance cascade from the eaten faces produces the whole 84-edge free ring. CORNER COLUMNS CLOSED by #1559 (2026-08-12): the "lens" was a near-coaxial cone×cylinder loop around the FULL cylinder arriving as OPEN marched chains; restrict kept open curves whole but only plane faces have a downstream trim, so the quarter-band got a chain far outside its window and the splitter minted the doubled garbage. Fix: `emit_curve_windows` (generalized from closed) windows open marched sections between two non-plane faces. Result: free 84→56, over 4→0, one clean 214-face shell, zero doubled groups (the remove_doubled_faces same-surface gate on branch fix/doubled-faces-same-surface-gate is now unexercised here — semantically right, unshipped). REMAINING 56 (z=0 + z=1.2 pocket/channel ring, one mechanism, both slab planes): the foot-ring fence lines on the holed slab faces overshoot into the pocket hole BY DESIGN (`clip_line_to_face_boundary` reads only the outer wire, and its holed-face calibration note says the hole weave needs WHOLE sections — do not pre-split there); the ring chains then dangle at (±2.4,±4.25) inside the hole, `plane_internal_line_loops` returns None on the degree/interior gates, and the general path leaves the face as disc+whole (iw=0, interior point over the pocket). SHIPPED 2026-08-12 in 3.2.31 (#1563): the internal-loops rescue (`rescue_hole_dangling_loops` — trims fence chains dangling inside an inner wire at their exact rim crossings and closes them with rim-arc connectors) + the rim-tangent union (weaves multiple rescued loops and the hole into ONE inner boundary). Chain: free 84 -> 4, over 4 -> 0. TWO durable lessons: (i) any consumer of OrientedPCurveEdge geometry MUST respect `forward` — domain_with_endpoints on swapped endpoints yields the COMPLEMENT arc (the union silently never fired until its span detection read through the flag); (ii) clip-level inner-wire trimming REFUTED — even endpoint-only single-window trimming regressed THREE foils (groove_chain, dovetail a1corner holecut, exact_coincident_lip_fuse): the holed-face weave requires untouched sections; do not retry any clip-level variant. CIRCLEINSERT CHAIN CLOSED 2026-08-12 (released 3.2.32, #1567): the last 4 free edges were a PHANTOM RAY CROSSING — the ray-cast classifier's holed-plane test used chord-sampled hole polygons, and a generic ray's crossing landed 0.03 inside the true r=10 pocket circle but outside the sampled polygon (sagitta ~0.09, graze band 1e-6), a clean-reading miscount flipping one quadrant's pad piece to Inside. FIX: FaceGeom::Planar keeps circular holes ANALYTIC (center+radius, exact containment/graze; gated to true Plane faces with parallel circle normals) — NO vote-logic change; honeycomb 4/4. Strict pin `circleinsert_socket_fuse_is_strictly_valid` ACTIVE, chain free=0 over=0. Do NOT retry vote-layer rules for on-plane samples: the honeycomb and circleinsert flip points are formally inseparable at that resolution (four rule variants each broke one side). Full session arc: free 84/over 4 -> 0/0 across #1554 (extrude winding + chamfer keep-side, 3.2.29), #1559 (open-curve windowing, 3.2.30), #1563 (hole-dangling rescue + rim-tangent union + forward-flag spans, 3.2.31), #1567 (analytic classifier holes, 3.2.32). TOOL-SIDE RE-MEASURE on 3.2.32 pins pending (same-day control per doctrine). Parked branch fix/doubled-faces-same-surface-gate remains unshipped (semantically right, unexercised post-#1559); (b) the 26-min 4x4 label-bracket row, unattributed; (c) the tool's `cornerRadius` scenario — booleans replay clean, capture executeBatch traffic next; (d) roundpocket4 CLOSED 2026-08-13: collateral of #1581's pocket-mouth island fix (the corner hole at a partial-overlap interface is the same promotion-path island configuration; attribution by bisect — still free=15 + fallback at 9d47b189, clean on 3.2.36), pinned `partial_overlap_corner_hole_interface_fuse_is_exact` (interface_fuse_winding.rs: no fallback + strict validity + 8 cylinder quarters + volume sum) |
| **#1570 spacer export timeout, REFRAMED — not a regression** | Bisected 2026-08-12 on captured operands (op0-op3 byte-identical across the 3.2.29/3.2.32 captures; only op4's body differs): the 3.2.28/29 "4s baseline" was a SILENT op3 analytic failure feeding a 662-face mesh blob (volume wrong +2.2) downstream, so the old speed is not a target — 3.2.30's open-curve windowing FIXED op3, and the exact body exposed op4's real defect: 8 corner pairs of opposed 45° cones (anti-parallel axes, 0.25mm apex offset) that the marcher shredded into 50-64 closed micro-loops each (12-15s native + free=41/over=18 → mesh fallback; ×3 in wasm = the 46s timeout). FIXED: equal-tangent parallel/anti-parallel offset cone-cone reduces to a radical-plane conic (`offset_parallel_cone_cone`, math `analytic_intersection.rs`; FF cone-cone arm now emits exact Ellipse sections) — RAW fuse 14.1s → 0.4s, free=41/over=18 → 12/4, native OPS ~3.4s incl. the still-firing mesh fallback (under the 30s tool budget). RESIDUE CLOSED 2026-08-13 (branch fix/coplanar-probe-own-region): the fuse is EXACT end-to-end — OPS 435ms no fallback, free=0/over=0, vol 2404.44@0.01, watertight at export tolerance. FOUR roots beyond the cone shred, each verified by its own instrument: (1) `classify_coincident_coplanar` depth probes walked tip->vertex-centroid, which for the annular foot plate lands in its OWN hole (the open cavity) — every probe read air-air and the buried ring classified Outside; probes now must lie in the sub-face's own region and the sampled interior point joins the candidates. (2) The splitter's general path carried sections lying wholly inside a hole into the arrangement whenever integrate_holes_plane BAILED (a section crossing the hole's corner ARC cannot be woven) — they closed into four phantom in-hole lens loops (garbage faces + punched holes); wholly-in-hole sections are now dropped, sampled arc-true via sample_wire_loop_uv_via_frame (per-edge sampling scrambles the polygon — wire chaining matters). (3) `shell_is_outward_oriented` sampled edges at raw t in [0,1], but evaluate_with_endpoints takes RAW curve parameters except Line — marched NURBS edges extrapolated to 1e21 coordinates and the vote's sign was noise (prior passes were coin flips); parameters now map through domain_with_endpoints. (4) The reversed flag is NOT a reliable orientation datum: boolean outputs circulate with MIXED flag-vs-winding populations (op3's body: 37/50 planar faces disagree) that every solid-level oracle accepts, while Cut-minted cavities are COHERENTLY flag-flipped — the vote now computes both the flag-composed and wire-only flux and arbitrates: agreement wins outright; disagreement with near-total reversed mass is a cavity (flags rule); mixed disagreement is flag rot (wires rule). Fixture `crates/io/tests/spacer_foot_fuse_inmem.rs` (strict: no fallback + validate + volume). DURABLE: never trust `is_reversed` alone for orientation-sensitive logic; the WINDING_CENSUS mode in replay_pair measures a solid's flag health. TOOL-SIDE CONFIRMED 2026-08-13 on released 3.2.35: spacer test 2.37s (was the 46s timeout; the broken-op3 3.2.28 "baseline" was 3.94s). Full-suite same-day pair on the identical 2794-test catalog: 3.2.35=146 failed vs 3.2.22=145 — flat aggregate, but the flips decompose cleanly: FIXED (mitsukude overhang, divider ramp footings, circle-insert STL, thin-wall STLs, O-shape half-sockets STL, three lidGripRelief rows); FLAKY across runs, not version-correlated (4x4-everything 56s, preferIdenticalPieces 35s, the wallCutoutBuilder ms-scale rows); and two "regressions" bisected to 3.2.34 that are BENIGN per-kernel snapshot drift (slotted-without-lip 1056->1060 tris, topOffset 2426->1856 — the exact radical-plane ellipse replaces dozens of marched fragments, so counts move; the doctrine already says snapshot counts are not defect signals; they refresh at the next tool pin bump) |
