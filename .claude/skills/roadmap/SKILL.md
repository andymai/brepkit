...
- **A turned ball, and a ball less a box corner (CLOSED 2026-09-25; pins in `crates/operations/tests/sphere_box_corner.rs`)**:
  ...
  (`build_box_sphere_octant` assumed a right-handed corner). That shortcut
  also wound all four wires clockwise, so a second boolean read its sphere
  patch's side backwards: a box corner cut from the octant was ignored and
  a rod through it measured 12.824527 against 12.816012.
