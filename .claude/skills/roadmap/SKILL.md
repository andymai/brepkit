...
| **Kumiko corner-window strut fuses: marched-section junction consistency** | Three roots shipped 2026-08-13 (#1594 sweep honesty + trim hardening on 3.2.39, #1596 quadric×NURBS sections in the pending 3.3.0): the ruled strut now serializes 40 NURBS + 2 planes, the wedge's cylinders split (3 sections → 2 pieces on the inner), and the failure is a clean assembly abort ("open growth shell", 7 faces = the wedge detaching). LIVE ROOT: the outer cylinder's 7 sections arrive at fill_face_info as two chains whose consecutive endpoints miss by ~2e-3, so no closed cut forms ("split into 1"). Those endpoints are NOT raw restrict output — an experiment widening the junction-registry trigger band to 5e-3 and snapping pass-through chain ends fired 3 times and moved nothing (REFUTED at that altitude): the 7 pieces are minted DOWNSTREAM, most likely `make_blocks`/`link_existing` splitting section pave-blocks at sampled-precision T-junctions. Next: instrument where the outer cylinder's section pave-blocks get their split points (`fill_face_info` consumes them) and weld THOSE to shared exact junctions. Ready-repro `kumiko_diagonal_strut_fuse_is_exact` (ignored, ruled fixture) is the acceptance bar; run the full splitter foil set on any change |
