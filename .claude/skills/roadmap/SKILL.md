...
| **Assembly cluster fuses degrade to mesh; the counterbored tapered tube exports open** (`assemblyGenerator.scenario.test.ts`: "arch at the base center" and "tilted block and cradle" throw `cluster fuse degraded to mesh fallback`, "a counterbored, tapered tube loses volume to the collar" exports 622 open mesh edges) | The three assembly-scenario failures left after the coaxial same-domain fix (3 of 23 vs the same-day 3.3.9 control's 6). Undug: capture the cluster fuse operands with an untracked `__kernel-tests__` probe (pattern: `captureScoopOps.test.ts`, wrap the raw kernel's booleans) and replay with `replay_pair` |
