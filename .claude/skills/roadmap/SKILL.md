...
**Inventory status (2026-08-15): ZERO deferred-defect pins** —
`kumiko_diagonal_strut_fuse_is_exact` is ACTIVE (the corner-window frontier
closed). Every remaining `#[ignore]` is an explicit diagnostic or a slow-test
marker. Known stale-but-harmless:
...
| **3.4.0 blend cutover: the junction fan mints twin radials at four-stripe mixed-radius corners** (`crates/blend/src/corner.rs` `build_junction_fan_faces`; fixture `crates/io/tests/groupedscoop_fillet_cut_inmem.rs`, two ignored ready-repros) | Verified root of the `groupedScoop` regression (4 vs 2 on 3.3.9); the other 3.4.0-only families (`solidCutouts` 1 vs 0, `combriser` 4/4 vs 0/4, `assemblyGenerator.scenario` 13 vs 6) fail the same way (open export meshes) but are UNATTRIBUTED until captured. The scoop tool is a 9-edge `filletVariable` with radii 2/0.6/0.263/0.393, the r=0.263 stripe's cross boundary is a sub-segment of the r=2 stripe's on the same support line, `boundary_cycle` accepts the walk that passes that vertex twice (it marks edges used, never vertices), and the fan then builds two radial lines between the same endpoints. The tool is manifold by edge id; the bin cut's endpoint-keyed duplicate merge collapses the twins into one four-owner edge, GFA returns over=1, and the mesh fallback exports 22..274 boundary edges. 3.3.9's engine closed the same junction with 28 faces and the cut stayed exact. REFUTED: requiring a simple cycle in `boundary_cycle` alone; the corner then fails outright and `fillet_variable` returns its open first assembly (free=44). The fix needs a closure for the pinched cycle (trim the long cross boundary at the short one's end, or fan each lobe with its own apex without the collinear overlap). Do not bump the tool past 3.3.9 before this closes |
