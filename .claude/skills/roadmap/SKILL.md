...
`prism_vertical_corner_fillets_are_watertight` (the assembly parts), and
...
| **Prism vertical-corner fillets (the assembly `block`/`comb`/`riser`/`boreBank` parts): four stripes ending on untouched caps close by edge id but are not watertight** (ready-repro `crates/wasm/src/bindings/assembly_probe_tests.rs` `prism_vertical_corner_fillets_are_watertight`, diagnostic `comb_part_probe` with `STAGE=`/`FLUX=`/`DUMP_WIRES=`/`LOG=`; mechanisms on branch `wip/prism-corner-fillets`) | Native replay of `assemblyPartTemplate.ts`: a 70x14x35 bar, r=2.5 on the four vertical edges, r=1 on the eight-edge top rim, then four slot cuts. Stage 1 on main: 4 cylinders + 14 planes, 32 open mesh edges; every later stage is collateral (rim 891 open, slots mesh-fallback). Three roots, each with a mechanism on the WIP branch and the fixture that rejects it: (1) a stripe's terminal end on an untouched cap is closed by a reversed planar runout patch overlaying the cap's corner, because the end-cap notch runs after the corner solver and only for one-edge planar fillets (`propagate_single_contact_splits`); the WIP splits the spoke at "notchable" terminals (planar cap, two straight corner edges) on every face and notches before the corner solver; rejected by `aggressive_radius_fillet_variable_is_watertight` (32 free edges) and, together with (3), by the gridfinity d5 canary. (2) the band builder winds two of the four mirrored corner bands clockwise around their surface normal, and with every original face rebuilt the seedless orientation repair pairs them by a flag toggle that inverts them (`normalize_face_normals` is rev-invariant, so it cannot see it); the WIP re-checks each new face's effective winding against its surface normal after propagation and triple flips; rejected by the `cross_one_row` volume oracle (64,968 within 0.1%). Seeding the repair on rebuilt originals instead was refuted by the same oracle. (3) the plane x cylinder analytic stripe classifies a rounded-prism cap as a plate around a post (`plane_is_bounded_disc` only knows a full disc), so the rim ease rounds the cove side at (±32.5, ±8, 35); the WIP adds a local material-side test at the spine (`plane_material_inside_cylinder`); rejected by cross_one_row and, with (1), the d5 canary. With all four on, stage 1 is exact and watertight (all fluxes positive) and the slots cut exact; the rim stage still keeps one trimmed corner cylinder inverted. Tool-side this is the `combriser` file and the remaining `assemblyGenerator.scenario` failures. Also refuted: a plan-level "tangent unselected edges are not sharp" filter (turns mixed-radius junctions over a smooth spoke into continuations with nothing to share: the aggressive scoop opens) |
