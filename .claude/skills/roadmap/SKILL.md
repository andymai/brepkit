...
| **Cone ∪ box "circle outside"** — needs ONE named component: a chain-band splitter | algo/GFA | `boolean::tests::diag_circle_outside_inventory` (ready-repro; its doc comment carries the full dig map). The cone's fuse boundary is a closed chain WINDING the lateral (4 corner ring-arcs + 4 NURBS wall arches). Two prerequisite steps are SHIPPED and behavior-neutral (winding veto on the internal-loops shortcut; seam-meridian anchoring of winding chains). Remaining: generalize `split_periodic_face_into_bands` from closed seam-anchored circles to a winding chain separator — the greedy and DCEL both fail on the chain's identical-tangent parallel twins, so walk the chain once and emit the two bands directly. Do NOT re-try DCEL-side fixes; the twin-parallel mistrace is structural. The tangency family proper (2- and 4-tangency, cone and cylinder) is CLOSED and pinned by `cone_union_box_should_be_analytic`, `cylinder_union_inscribed_box_is_analytic_watertight`, `two_tangency_box_fuse_is_analytic_watertight` |
