...
|---|---|
| **2x2 label bracket perf row: scenario-first 2.3x slower** | Matrix 2026-08-13 on shipped 3.2.36 read 223 vs 80ms; probe decomposition (labelBracketPerf.test.ts pattern, untracked): process-cold is parity (420 vs 374ms), scenario-first in a warm process is **278 vs 119ms** — the real gap, ~160ms of extra first-generation boolean work — and warm repeats (6 vs 22ms, brepkit 4x faster) are brepjs parameter-cache hits on both sides, so ONLY scenario-first numbers are kernel comparisons in this harness. Geometry clean both sides (brepkit nm=0; the reference itself nm=147). Also from that matrix: bp 6x4 magnets improved to 1.11x, 4x4 mag no-lip exact 1.00x, aggregate 0.63x; the three volume-FAIL rows (3x3 scoop+label+lip, compartments+scoop, full-featured) are all the reference double-cover over-count (nm 959-1802 vs brepkit 0) — not defects. Next: capture the scenario's boolean traffic and replay natively for the hot op (`parity-benchmarking` capture recipe; the #1510 bracket FUSE was 39ms native when closed — suspect a different op in the chain first) |
