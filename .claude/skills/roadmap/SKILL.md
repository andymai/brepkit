...
| **STEP export drops a solid's cavities** (`crates/io/src/step/writer.rs` `write_solid`) | Only the outer shell is written (a `MANIFOLD_SOLID_BREP`) and the reader builds each solid with no inner shells, so a box with a ball cut from inside it loses its cavity on a round trip (the box `[-5, 5]³` less a ball of radius 2 at its centre reads back 1000.0000 against 966.4897). Next: write a `BREP_WITH_VOIDS` with `ORIENTED_CLOSED_SHELL` voids and read it back |
