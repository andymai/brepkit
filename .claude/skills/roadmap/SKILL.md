...
**Inventory status (2026-09-15): FOUR deferred-defect pins**, each owned by an
OPEN row: `groupedscoop_tool_tessellates_watertight` +
`groupedscoop_cut_removes_at_most_the_tool` (the two-stripe corner model) and
the two kumiko wrap cuts (`kumiko_wrap_strut_cut_inmem.rs`,
`kumiko_wrap_lattice_cut_inmem.rs`). Every other `#[ignore]` is an explicit
...
| **Assembly cluster fuses degrade to mesh** (`assemblyGenerator.scenario.test.ts`: "arch at the base center" and "tilted block and cradle" throw `cluster fuse degraded to mesh fallback`; operands captured with the `captureScoopOps.test.ts` wrapper pattern, replayed with `replay_pair FUSE_ALL=1 FUSE_MEMBERS=<third member>`) | TILTED only, since the arch closed (Closed entry below): block x cradle (two tilted parts overlapping 5 mm in x) fails GFA ("open growth shell with 18 faces would be dropped") and the mesh fallback is non-manifold; plate x cradle is accepted closed by id with 100 open mesh edges and a volume 2037 mm³ below the raw result. Block x cradle alone (`BK_OPEN_SHELL=1`): the 18-face open shell is the cradle's part outside the block, and its free edges are the cradle's cut boundary on the block's +x face and the block's -y corner cylinder (a line, the groove arcs and two ellipse arcs at x=8): those block faces were never split there, six EF crossings of the cradle's rounded-edge curves with the +x face having been dropped as `outside face boundary`. TUBE: the test passes since the plate x tube nesting closed, but its mouth chamfer (`chamfer(tube, both rims at z=60, 1/3)`) fails cleanly and the tool keeps the sharp mouth: the planar bevel throws `cannot normalize zero vector` (a closed circle's single vertex gives a zero direction) and the blend engine's chamfer builds both cone bands but leaves the plane and cylinders untrimmed (8 faces, every band edge owned once, 286 open mesh edges): `ChamferBuilder` trims through endpoint-sampled planar trims (`sample_nurbs_endpoints`) that cannot take a closed rim contact, the periodic-contour machinery lives only in the corrected `FilletBuilder`; native probes on a `make_cylinder` tube minus a coaxial counterbore, parked in the session scratchpad. The six dropped EF crossings are correct (those cradle curves cross x=8 beyond the block's rounded corners). SUB-ROOT 1 FOUND: the block's +x face reached the splitter with its two straight NURBS boundary edges (the fillet contact lines) unsplit, so its five sections could anchor on no boundary vertex and were all dropped; `boundary_edges_to_pcurve_with_images` expands a NURBS edge only when circle-like, and a straight NURBS edge with a weld-coincident pave junction now expands too on branch `fix/nurbs-line-boundary-expansion` (parked: with it the pair assembles 34 faces but 26 free edges remain from plane sections overshooting into the rounded-corner region and tiny marched pieces where the cradle's 0.8 mm rounded edge meets the block's corner cylinder; no primitive pin discriminates the expansion yet, an axis-aligned or tilted NURBS-edged box splits fine either way). Next: those two remainders on the captured pair (`asmcap/tilted`, `replay_pair OP=fuse BK_OPEN_SHELL=1`) |
...
| **Tool end face coplanar with a fragmented knuckle end** (`hingeSwing` op3029 second tool; scratchpad `pincut/bin.bin` x `tool1.bin`) | The long hinge pin ends exactly on the journal end plane (x = -47.905) where the knuckle's end is already three coplanar sub-faces (bracket end, disc wedge, disc remainder) and the pin's own end cap is fragmented (half-disc, slot pieces). The coplanar FF hands the cap fragments' internal edges to the wedge, the arrangement emits a triangle inside the bore, and two kept faces keep its sides: 3 free edges after the classifier fix. The sub-micron slivers at x = 5.323/5.328 (a tool end 5 um off a knuckle face) belong to the same family. Undug |
| **Periodic band with the seam inside a section-bounded sector** (repro: rod r2.2 h8 fused with box 3x3x8 at the origin, bored coaxially r0.9, then cut by a slot box 1.2x0.6 straddling the bore's +x seam; the `cut_a_pin_bore_through_a_knuckle_flush_with_its_bracket` pin stops before the slot) | The bored knuckle's bore wall is a clean band (three bottom arcs, seam, three top arcs, seam; `FACE_WIRES=3` on the bored operand), but once the slot's rulings pave its rims the image expansion (`boundary_edges_to_pcurve_with_images`) hands the splitter rim pieces with wrong periodic UV spans, some in both directions (`BK_SPLIT_TRACE=<bore wall>`: a top-rim piece spanning u 0 to 4.37, twins of the 2.36 to 4.37 piece), so the arrangement traces one loop and the assembler sees a rim arc used three times. Same family as the closed-rim work in #1666; the expansion's UV assignment for reversed arcs on a periodic face is the place to look |
| **Partially overlapping coplanar end caps in a fuse** (repro: cylinder r0.9 h8.01 fused with a box 0.6x1.2x8.01 whose end faces are flush with the cylinder's and whose footprint crosses the rim) | The disc keeps the box's inner rectangle as a hole boundary and the box's outside piece keeps the rim arc, both free: the coplanar same-domain pass treats a partial overlap as a subtraction. Likely the `op59` lid plate family (the plate top loses its outer rim after the fuse with the lid body's relief). Undug |
...
- **Knuckle pin bore: a coaxial bore cut through a rod fused to an on-axis bracket (CLOSED 2026-09-15; `hingeSwing` op3029 first tool, the bin-blob root of the swing sweep)** —
  ...
  `cut_a_pin_bore_through_a_knuckle_flush_with_its_bracket`.
