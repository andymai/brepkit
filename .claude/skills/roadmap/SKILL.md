...
| **Feature recognition (Beta)** | Dihedrals are signed from outward normals and the edge tangent, adjacency reads every wire and shell, holes are concave cylinders, a pocket's floor is its face with the most concave neighbours, a fillet is a curved face tangent to two neighbours, a chamfer must be small beside the faces it bevels (tests in `feature_recognition.rs`). Still heuristic: a stepped pocket reports one floor and its landings as walls (`Feature::Pocket` has a single `floor`); a chamfer larger than half a bevelled face is missed; a curved channel floor tangent to both walls reads as a fillet |
