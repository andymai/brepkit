...
- **A ball less a box corner, and the box-sphere octant (CLOSED 2026-09-25; pins in `crates/operations/tests/sphere_box_corner.rs`)**:
  ...
  the axis; it reads the wire's turn in u now. The collar arrangement
  (`split_noseam_by_arrangement`) keeps the region inside every arc chain,
  the whole answer only when three or more chains surround the pole; with
  one or two (a half-space, a slab) it dropped the rest, and such a face
  now fails (a mesh fallback). A plane face's arc filter no longer widens
  its band by a long straight side's sagitta.
