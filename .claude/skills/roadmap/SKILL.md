...
| **Periodic band with the seam inside a section-bounded sector** (repro: rod r2.2 h8 fused with box 3x3x8 at the origin, bored coaxially r0.9, then cut by a slot box 1.2x0.6 straddling the bore's +x seam; the `cut_a_pin_bore_through_a_knuckle_flush_with_its_bracket` pin stops before the slot) | The bore wall's kept band is bounded by the slot's two rulings; with the seam inside the removed strip (or inside the kept band) the sector splitter's rim walk emits one rim arc three times and the cut falls back. Same family as the closed-rim work in #1666; undug |
