...
| **Sphere measure leftovers: the chordal equator** | A solid bounded by a ball zone between the chordal equator and a section circle measures off: `make_torus(4, 1.5)` less `make_sphere(3)` reads 166.4567 against 166.3658 (`torus_coaxial_tools.rs` bounds it at 1e-3), and `classify_point` on that Cut reads (2.8, 0, 0), on the equator plane, Inside where (2.8, 0, 0.3) reads Outside; a ball octant (its sphere face bounded by meridians and equator chords) measures 14.13664 against 14.13717. Both trace to the hemispheres meeting on chords (the quirk row below). A sphere face whose outer loop reaches a pole still takes the old (u, v)-box flux in `analytic_sphere_signed_volume` (its exact area comes from `sphere_face_uv_area`). Next idea for the cone seam row above: trace a cone in its unrolled window [u_s, u_s + 2 pi] with non-periodic keys so a seam vertex's two copies stay distinct and the piece the seam crosses becomes two faces sharing the seam edge; a pointed cone needs a synthetic apex edge to close (the DCEL retry on every pointed cone changed nothing) |
...
- **Sphere faces trimmed by a box or a rod: area, mesh and volume (CLOSED 2026-09-25; pins in `crates/operations/tests/sphere_face_area.rs`)**:
  ...
  the face's mesh choosing the patch or the sphere past it), a trimmed or
  pole-touching face meshes through the constrained CDT, and a face whose
  outer loop clears the poles takes the patch flux (`sphere_patch_flux`).
