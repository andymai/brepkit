...
| **A column with a corner inside the ball falls back once offset and turned, and the ball less it always** (`make_box(4.5, 4.5, 10)`: at `(-2.5, -2.5, -5)` the Intersect and the column less the ball are exact, the ball less it falls back to 348 faces; at `(-2.6, -2.2, -5)` turned 0.2 about `z` all three fall back, 494, 342 and 497 faces) | Undug, on main as well. The walls `x = 2` and `y = 2` meet on the sphere, so the region past them is bounded by one seam arc longer than half a turn and two wall arcs |
| **A tilted column through the ball meshes open and measures off** (`make_box(5, 5, 10)` at `(-2.5, -2.5, -5)` turned `rotation_x(0.3) * rotation_y(0.2)`) | Undug, on main as well. The ball within it and the column less the ball are exact and valid but not watertight, and measure 104.40620 and 145.58035 against 104.19624 and 145.80376. A column ending under the pole and tilted `rotation_x(-0.2)` (top at `z = 2.93093`) or `rotation_x(-0.35)` (top at `3 cos(0.365)`) gives the same two results exact, valid and within `1e-3`, but not watertight (pinned without the mesh check in `a_column_ending_in_the_ball_keeps_the_cap_over_it`). With the ball turned `rotation_y(0.3)` then `rotation_z(1)` and the column fused with the rod of radius 0.1 at `(2.75, 0)`, the tool less the ball is exact and valid but not watertight and reads 145.9082 (the rod pin's closed form gives about 146.04), and the ball less the tool and within it fall back |
...
| **A sphere face holding the pole within a boundary that dips meshes past its region** (`make_sphere(3, 32)` less the column `|x|, |y| < 2.2`, `-5 < z < 2`, whose top rim pierces the ball) | The Cut is exact: ten faces whose areas match their closed forms (the top 15.631, each wall 13.048), and whose sphere faces sum to the 79.05 the divergence theorem needs for the truth 30.3591. The sphere face holding the pole (the cap over the column joined to the four side caps' upper halves by sixteen arcs) meshes on its own to 50.2209 against its `face_area` of 48.8942, so `solid_volume`, which meshes sphere faces, reads 31.8122 and the solid's mesh is open; the check crate reads that face as 37.6991. Main falls back to a 545-face mesh reading 29.2408. Undug |
| **A section through a ball's pole leaves a face that measures short** (`make_sphere(3, 32)` turned `rotation_x(0.35)`, within `make_box(5, 5, 5 + top)` at `(-2.5, -2.5, -5)` with `top = 3 cos(0.35)`, so the column's rim passes through the ball's pole) | The Intersect is exact and valid and the engine's ray cast reads it right, but it meshes open and `solid_volume` reads 88.4143 against 103.8907; with the rim 0.0003 rad off the pole it reads within 2e-3. Undug |
...
| **The check crate integrates a plane face through its boundary's chords** (`crates/check/src/properties/face_integrator.rs` `integrate_planar_face`, sampled by `crates/check/src/util.rs` `wire_polygon`) | A plane face's area, flux and moments come from its wires sampled into 32 chords per curved edge, so a curved boundary reads short by its sagitta segments: the keyhole cap in `crates/operations/tests/extrude_major_arcs.rs` (a 323-degree arc of radius 1.58) reads 28.2368 where the truth is 28.2004, a 32-chord circle holds 0.64% less than its disc, and `make_cone(1, 0, 2)` tipped onto its side with its base disc in the plane `x = 1` reads 2.101111 against 2π/3 = 2.094395. `operations::measure::solid_volume` sends solids with a bored quadric face through this integrator. Green's theorem along each edge's own parameter would read a curved boundary exactly |
