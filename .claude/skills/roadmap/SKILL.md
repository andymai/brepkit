...
Chase the 849s. MEASURED SPLIT: `generateBin` alone is **847s** of the 849 — tessellation and STL are noise, so this is the boolean chain, not the mesher. Native `compound_cut` on the captured operands is only 11.8s (F=1146, clean), so one call is not the story. STRUCTURE (code, `kumikoWrapBuilder.ts` ~line 411): the carve loops `cutAll(region, family)` once PER FAMILY over an ACCUMULATING region, and that whole function runs per perimeter chunk — so there are many such calls, each starting from a more complex region than the capture. MEASURED (wrapped `cutAll` via vi.mock across a whole goma generation): **32 calls, ZERO failures** — so there is NO bisect thrash — in 4 chunks x 8 families (tool counts 4,6,7,180,6,3,14,80 repeated 4x). Summed cutAll time is only **206s of 911s = 23%**, so **~705s (77%) of goma generation is NOT in cutAll at all** and is still unlocated (candidates: the per-strut sketch/extrude/translate prism construction, `fuseAllOrNull` in wallPatternClips, or other kernel ops the probe did not wrap). Batching is real but SECONDARY: native chunked replay (`CHUNK=n` in `crates/io/examples/replay_kumiko_goma.rs`) gives 1x180 = 11.7s vs 3x60 = 15.8s vs 6x30 = 18.6s, with the SAME 30 tools costing 393ms at F=168 and 6103ms at F=1131 — cost grows steeply with region complexity, so the per-family loop is the pessimal shape, but fixing it cannot recover the missing 77%. Also unexplained: that 180-tool call is 33.5s in-tool vs 11.7s natively (3x). NEXT PROBE: wrap the REST of the brepjs ops, not just cutAll. REFUTED: the captured goma `compound_cut`
