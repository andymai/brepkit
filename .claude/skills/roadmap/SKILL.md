...
|---|---|
| **Label-bracket fuse opens the shell and mesh-falls-back (#1510)** | ROOT FOUND, unfixed. `fuse(bin, bracket)` returns 10 free edges so the ops layer pays the mesh fallback (121 all-planar vs 57 with 8 cylinders; 95 of the row's 124ms of boolean time). The bracket's SQUARE back corners intrude into the bin's ROUNDED cavity corners, and the cavity cylinder is tangent to y=40.550 at x=+-38, so the z=16.000 split exists only for \|x\|<38; the complement sub-face straddles the bin boundary (columns inside, top strip outside), one verdict drops it all. Fix belongs at the graze/section level, not SD. Repro `crates/io/tests/labelbracket_fuse_inmem.rs` (~11ms) |
