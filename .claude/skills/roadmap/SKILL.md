...
| **Reversed sphere caps: the solid mesh and the wire convention** | A box `[-5, 5]³` less a ball of radius 2 at `(0, 0, 5.5)` leaves a reversed sphere face bounded by one circle (a dimple): `solid_volume` is exact (989.397125), but the solid mesh reads 937.98, because `close_loop_at_pole` (`tessellate/nonplanar.rs`) takes a reversed face's wire as running about the face's normal, while the GFA builder flips the flag and keeps the wire about the surface's outward normal. `transform.rs` near line 218 flips on the same assumption, and the STEP reader keeps a `.F.` face's loops as written (about the face normal by the standard). Next: one convention across the three, the mesher first |
