...
| **The engine's ray cast reads a ball's collar by its flat polygon** (the ball within the column above) | `classify_ray_cast` misreads 33 of 120 random points of the ball within the column (the check crate's classifier none): `sphere_face_loops` gives up on the collar and the face takes the flat-polygon fallback, so a ray from `(0.3, 0.2, 2.5)` up through the collar counts no crossing. First suspect: the check that each arc is the whole of its circle on the region's side, since the collar's four seam arcs share the equator circle |
