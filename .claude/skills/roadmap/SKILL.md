...
| **Turned or mirrored balls: the remaining fallbacks** (audit probe `zz_pose_audit` in the session scratchpad) | Safe fallbacks. A ball turned or mirrored less a rod along `y` (radius 0.6 through `(0.5, _, 1)`) falls back, as does a ball against the half-space `x > 0.5` in every pose (the chordal-equator TERMINAL entry). The slab Cut upright is exact but invalid: its two pieces share one shell, the representation the cylinder and cone slab Cuts share |
...
- **Turned or mirrored balls fell back (CLOSED 2026-09-25; pin `turned_and_mirrored_balls_stay_exact` in `crates/operations/tests/sphere_box_corner.rs`)**:
  ...
  Gauss integrator integrated a zero band for a turned equator. The corner,
  the rod's Intersect and the slab are exact turned and mirrored.
