...
| **A rod through a plate's edge with its seam through a window corner can fall back; some exact cuts mesh open** (the reviewer's battery, `rev1786-87/probes/zz_probe_1787.rs` in the session scratchpad) | On main as well: turned `-acos(-x)` the rod at `x = -0.3`, `-0.1` and `0.4` falls back in every op, and at `x = -0.5` turned `acos(0.5)` the rod less the plate and the Fuse do, each mesh up to 2.1% off. Fourteen results are exact and within `1e-6` but mesh open, among them the rod at `x = 0.4` turned 2 less the plate and the Intersects with a plate turned 0.5 about `y`; main had each open, wrong or falling back |
