...
| **A box holding the pole less a bored ball drops the bore's post** | `make_box(10, 10, 10)` at `(-0.7, -1.1, 0.1)` less `make_sphere(3, 32)` bored through its poles by a coaxial `make_cylinder(0.3, 10)` from `z = -5` reads 972.657217 with 7 faces and no cylinder, exactly the box less the plain ball: the rod's piece inside the box (about 0.818) should stay as a post. The bored ball alone measures 111.405124 with 3 faces. Silently wrong, no fallback |
