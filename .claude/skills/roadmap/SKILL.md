...
| **Second booleans on a ball's octant: the remaining fallbacks** (pin `box_octant_feeds_a_second_boolean` in `crates/operations/tests/sphere_box_corner.rs`) | Safe fallbacks. The octant above the equator less a rod of radius 0.4 along `z` through `(1, 1)` falls back to a mesh, whether the box shortcut or the boolean engine built it (below the equator it is exact). The octant below the equator with box and ball turned 0.3 about `z` falls back in its first boolean (94 faces, 13.937 against 14.137) |
