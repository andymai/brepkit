...
| **The check crate's integrator misreads plane faces with curved edges, a ball whose wire runs along its seam, and a cone less a quadrant** (`crates/check/src/properties/face_integrator.rs`; plane faces sampled by `crates/check/src/util.rs` `wire_polygon`; pole pin `crates/operations/tests/check_face_bounds.rs`) | A plane face's area, flux and moments come from its wires sampled into 32 chords per curved edge, so a curved boundary reads short by its sagitta segments: the keyhole cap in `crates/operations/tests/extrude_major_arcs.rs` (a 323-degree arc of radius 1.58) reads 28.2368 where the truth is 28.2004, and a 32-chord circle holds 0.64% less than its disc. `operations::measure::solid_volume` sends solids with a bored quadric face through this integrator. Green's theorem along each edge's own parameter would read a curved boundary exactly. On main as well: `make_sphere(1, 16)` less the box over the positive octant reads as the whole ball (4.188790 against 7π/6 = 3.665191) until turned, when its volume is exact but its centre of mass across the ball's axis is not (turned 37 degrees about z: x -0.008893, y -0.078741 against -0.010544, -0.075024); `make_cone(1, 0, 2)` less the box over `x, y > 0` reads 2.814929 against 1.570796 |
...
- **Extruding an arc past half a turn built an inside-out wall (CLOSED 2026-09-26; pins in `crates/operations/tests/extrude_major_arcs.rs`)**:
  ...
  the arc lies on now, and the cylinder wall's volume readers and the
  per-face tessellator walk each arc along its own span.
