...
fn miter_sweep_keeps_offset_profile_position() {
    // The miter family joins the as-positioned contract: an offset
    // perpendicular profile swept along a kinked path stays where it lies.
    let mut topo = Topology::new();
    let t = 1e-7;
    let v0 = topo.add_vertex(Vertex::new(Point3::new(5.0, 5.0, 0.0), t));
    let v1 = topo.add_vertex(Vertex::new(Point3::new(6.0, 5.0, 0.0), t));
    ...
    assert!(
        (bb.max.z() - 10.0).abs() < 0.6,
        "sweep must span the full kinked path, got {bb:?}"
    );
}
