    ...
    for &fid in shell.faces() {
        ...
        match face.surface() {
            ...
            FaceSurface::Plane { normal, .. } => {
                // Exact divergence-theorem volume for planar faces.
                // Properly handles is_reversed and inner wires (holes).
                let eff_normal = if face.is_reversed() {
                    -*normal
                } else {
                    *normal
                };
                // Compute signed area of outer wire via Newell's method.
                let wire = topo.wire(face.outer_wire())?;
                ...
                if verts.len() >= 3 {
                    ...
                    for i in 0..n {
                        let j = (i + 1) % n;
                        cx += verts[i].y() * verts[j].z() - verts[i].z() * verts[j].y();
                        cy += verts[i].z() * verts[j].x() - verts[i].x() * verts[j].z();
                        cz += verts[i].x() * verts[j].y() - verts[i].y() * verts[j].x();
                    }
                    let area_vec = Vec3::new(cx, cy, cz);
                    let signed_area = area_vec.dot(eff_normal) / 2.0;
                    let mut net_area = signed_area.abs();

                    // Subtract inner wire (hole) areas.
                    // Only for Line-edge holes; curved holes (circles from
                    // cylinder cuts) need tessellation for accurate area.
                    let mut has_curved_hole = false;
                    for &iw_id in face.inner_wires() {
                        ...
                        if all_lines {
                            ...
                            for i in 0..hn {
                                let j = (i + 1) % hn;
                                hx += hole_verts[i].y() * hole_verts[j].z()
                                    - hole_verts[i].z() * hole_verts[j].y();
                                hy += hole_verts[i].z() * hole_verts[j].x()
                                    - hole_verts[i].x() * hole_verts[j].z();
                                hz += hole_verts[i].x() * hole_verts[j].y()
                                    - hole_verts[i].y() * hole_verts[j].x();
                            }
                    ...
                    if !has_curved_hole {
                        let d = crate::dot_normal_point(eff_normal, verts[0]);
                        total += d * net_area * 2.0; // * 6.0 / 3.0 = * 2.0
                        continue;
                    }
                    // Fall through to tessellation for curved holes.
                }
                // Fall through to tessellation if < 3 verts or curved holes.
            }
            FaceSurface::Nurbs(_) => {}
        }
