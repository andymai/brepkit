//! All four top edges of the 1 inch cube build at the exact limit `r = S/2`
//! (the pillow), and across the whole admissible range `(0, S/2]`.
//!
//! Each shared-face (top) contact is trimmed once from each of its two
//! corners' ends, so at `r = S/2` the *second* trim finds exactly `radius` of
//! contact left, and trimming a curve by its whole remaining parameter range
//! would produce a single-control-point NURBS the builder rejects. The
//! contact collapses instead: removed from its own stripe's wire (leaving
//! that stripe bounded by its bottom contact and its two corner creases) and
//! restricted in place to the corners' shared meeting vertex in the shared
//! face's wire, whose then point-only boundary the result-shell assembly
//! drops. All four corners' meeting vertices coincide at the limit
...
//! sweep across `(0, S/2]` including the `~12.446` transition, and the
//! `r = S/2 + 1e-6` control, which must still refuse with the source
//! preserved.
...
/// Across `(0, S/2]`: every radius builds valid/closed/oriented, and its
/// volume sits on the closed form. No point in range may refuse or come back
/// `Ok`-but-open. The sample set is both ends of the range, the `~12.446`
/// construction transition with one point either side of it, and three
/// interior radii.
#[test]
fn the_admissible_range_never_refuses_or_leaves_an_open_shell() {
    let radii: [f64; 8] = [0.5, 3.0, 7.0, 11.0, 12.445, 12.446, 12.446_000_1, HALF];
    for radius in radii {
        let mut topo = Topology::new();
