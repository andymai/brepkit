//! Equal-radius two-edge miter corners must have incident wires and closed
//! supports.
...
///
/// One size and one radius fraction: scale and radius sweeps belong to the
/// exact-limit and round-trip regressions, while what varies here is the
/// source construction, the rigid placement, the selection order and the
/// edge count (1 is the single-edge control, 2 and 4 the miter cases).
#[test]
fn native_contact_matrix_has_no_support_wire_crossings() {
    let size = 254.0;
    let fraction = 0.1;
        ...
        for transformed in [false, true] {
            for reverse in [false, true] {
                for count in [1, 2, 4] {
                    ...
                    let input = if boxed {
                        brepkit_operations::primitives::make_box(&mut topo, size, size, size)
                            .unwrap()
                    } else {
                        source(&mut topo, size)
                    };
                    let matrix = if transformed {
                        brepkit_math::mat::Mat4::translation(3.0 * size, -2.0 * size, 0.7 * size)
                            * brepkit_math::mat::Mat4::rotation_y(0.47)
                            * brepkit_math::mat::Mat4::rotation_z(0.31)
                    } else {
                        brepkit_math::mat::Mat4::identity()
                    };
                    brepkit_operations::transform::transform_solid(&mut topo, input, &matrix)
                        .unwrap();
                    let corners = [
                        Point3::new(0., 0., size),
                    ...
                    if count != 1 {
                        exact_product_oracle(&topo, result_solid, size, fraction, count, &inverse);
                    }
...
fn exact_product_oracle(
    ...
    // The sharp-mitered two-edge corner has no corner face, no torus and no
    // NURBS surface at all: every blend surface is an exact radius-r
    // cylinder, the two cylinders at each corner meet along one exact
    // `Ellipse3D` crease edge, and there is no top-contact crossing (checked
    // independently by this test's own outer loop) and no corner-fill arc
    // left over.
    assert_eq!(
        ...
        "V/E/F must match the miter construction's exact counts"
    );
    ...
    for &fid in &faces {
        ...
        assert!(
            ...
            "no torus: the miter corner never falls back to a horn torus"
        );
        assert!(
            !matches!(face.surface(), FaceSurface::Nurbs(_)),
            "no NURBS corner face: the miter corner has no corner face at all"
        );
    ...
    // product in [0,1) (convex, and fully tangent only at the isolated
    // top-contact meeting vertex), which makes the crease the true, convex,
    // non-degenerate intersection of both cylinders rather than a
...
/// Every selection order and rotation of the same edge set produces the same
/// geometry, a repeat of the same request produces it again, and a refused
/// radius leaves the source untouched.
#[test]
fn orders_repeats_and_refusal_preserve_sources() {
    let size = 254.;
    let points = [
        Point3::new(0., 0., size),
        Point3::new(size, 0., size),
...
/// The equal-radius planar two-edge corner's admissibility bound comes from
/// real per-stripe geometry. Case A (`count == 2`, two adjacent top edges:
/// only the shared vertex is a qualifying trihedral corner, the far ends are
/// plain terminals) is bounded by the stripe's own length. Case B
/// (`count == 4`, all four top edges: every vertex is a qualifying corner, so
/// every edge is consumed from both ends) is bounded by half that length.
///
/// Both exact boundaries are admissible, since the limit construction closes
/// the corner around the vanished material, and both must build on their
/// closed form.
#[test]
fn radius_admissibility_bound_matches_derivation() {
        ...
        for (count, max_radius) in [(2usize, size), (4usize, size / 2.0)] {
            for shift in 0..count {
                for reverse in [false, true] {
                    ...
                    // but strictly positive and non-degenerate, so the
                    ...
                    // exactly zero, and the limit construction closes around
                    // the vanished material
                    // (`close_collapsed_miter_corner`), so the two-edge case
                    // must build one valid, closed, oriented solid on the
                    // closed form `(2/3) size^3`. The four-edge pillow closes
                    // the same way (the second trim per shared-face contact
                    // collapses it into the corners' shared meeting vertex,
                    // and the zero-area top face drops out of the shell) on
                    // the closed form `(5/6) size^3`.
...
/// Case A and case B have genuinely different admissible radii on the
...
#[test]
fn case_a_and_case_b_have_distinct_bounds_on_the_same_edge() {
    let size = 254.0;
    let points = [
        Point3::new(0., 0., size),
        Point3::new(size, 0., size),
