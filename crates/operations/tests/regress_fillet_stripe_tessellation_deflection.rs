//! Regression: trimmed fillet stripes honour the requested mesh deflection,
//! including the stripes that meet at a sharp-mitered corner.
//!
//! All 24 cases (box/extrusion source, identity/rigid pose, single/adjacent/
//! four-edge selection, both radii) assert the deflection contract on every
//! cylindrical stripe face: triangle vertices, edges and barycentric
//! interiors sample the exact cylinder within the requested tolerance; the
//! mesh is watertight and manifold (the crease between two mitered stripes is
//! one shared edge chain, not a crack); every sample stays inside the
//! retained stripe domain; UV triangles neither overlap, reverse, nor leave
//! the analytic trimmed area uncovered; and each selected edge yields exactly
//! one cylindrical stripe face carrying its miter crease. Triangle counts are
//! diagnostic only and never an acceptance oracle.
//!
//! The defect this pins is a mesh defect, not a B-Rep one: a stripe face is
//! triangulated from its boundary samples, and in raw UV (radians against
//! model units) a long axial stripe joins opposite angular contacts across
//! the curved interior, sagging 0.74 mm against a 0.25 mm request at r = 2.54.
//! The shape of the mitered corner itself is pinned by the miter corner
//! matrix, torus-incidence and STEP round-trip suites.
