//! Two adjacent top edges of the 1 inch cube build at the exact limit
//! `r = S`, and across the whole admissible range `(0, S]`.
//!
//! At the limit the ball consumes the retained third edge, each stripe's
//! shared-face contact and the shared face's remnant, so the corner closes on
//! the two stripe faces alone, and each runout cap has to notch the blend's
//! cross-section arc out of the right two-edge corner path (at the limit both
//! of a cap's corner paths span the arc's endpoints). Each stripe is then the
//! trimmed cylinder patch bounded by one NURBS side contact, the miter's exact
//! `Ellipse3D` crease and one terminal circle arc; its area is exactly `S^2`
//! (untrimmed quarter cylinder `(pi/2)S^2` less the crease-plane trim
//! `S^2(pi/2 - 1)`), and it must mesh and integrate as that patch.
//!
...
//! sweep across `(0, S]` including the `~12.446` transition, and the
//! `r = S + 1e-6` control, which must still refuse with the source preserved.
...
/// Across `(0, S]`: every radius builds valid/closed/oriented, and its volume
/// sits on the closed form. No point in range may refuse or come back
...
fn the_admissible_range_never_refuses_or_leaves_an_open_shell() {
    let radii: [f64; 8] = [0.5, 5.0, 12.445, 12.446, 12.446_000_1, 19.0, 24.0, S];
    for radius in radii {
        let mut topo = Topology::new();
