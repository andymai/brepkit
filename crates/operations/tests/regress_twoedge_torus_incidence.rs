...
//! supports, no torus, and no corner face of any kind.
...
#[test]
fn two_edge_and_face_loop_have_incident_closed_boundaries() {
        ...
        for fraction in [0.05, 0.1] {
            for count in [2, 4] {
                for reverse in [false, true] {
                    ...
                    assert_eq!(tori, 0, "no horn torus");
                    // The sharp-mitered corner has no corner face of any kind
                    // (rational NURBS included) and no torus, only the two
                    // stripe cylinders and their shared crease.
                    assert_eq!(
                        rational_corners, 0,
                        "no corner face at all, rational or otherwise"
                    );
                    assert_eq!(
                        (
...
#[test]
fn planar_support_contact_boundaries_do_not_self_intersect() {
