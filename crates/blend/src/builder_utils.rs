...
pub fn notch_face_corner_with_arc(
    ...
    for i in 0..n {
        ...
        // Below the limit only one path ever spans `va -> vb`, and its
        // corner is the cap's own corner, at distance `r*sqrt(2)` from the
        // center, always outside the circle. At the exact limit `r = S` the
        // consumed corner has collapsed onto the arc's endpoint vertices, so
        // *both* of the cap's two-edge paths span `va -> vb` and wiring order
        // alone would replace the kept side. Judge the candidate by the arc's
        // own circle instead: replace the path whose corner is outside it,
        // never the path through its center.
        if let Ok(arc_edge) = topo.edge(arc_eid)
            && let EdgeCurve::Circle(circle) = arc_edge.curve()
        {
            let corner = topo.vertex(e0)?.point();
            if (corner - circle.center()).length() <= circle.radius() * (1.0 + 1e-6) {
                continue;
            }
        }
