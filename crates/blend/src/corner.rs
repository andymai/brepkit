...
fn multi_edge_corner_data(
    ...
    // Record which of the two trihedral constructions this junction is in,
    // so `compute_sphere_center` asserts its requested-radius theorem only
    // where that theorem holds (see `setback_trimmed_classification`).
    let setback_trimmed =
...
/// A junction vertex qualifying for the sharp-mitered two-edge corner
/// class: exactly two incident contours, equal constant radius, a
/// trihedral (three-plane) orthogonal convex corner.
...
/// Find every junction in `plan` qualifying for the sharp-mitered
/// two-edge corner class. Shared by
...
/// orphaned), so both always agree on exactly the same qualifying set.
pub fn find_equal_two_edge_miter_junctions(
    topo: &Topology,
    plan: &FilletPlan,
) -> Vec<MiterJunction> {
    let mut miters = Vec::new();
    'junctions: for junction in &plan.junctions {
        ...
        let Some(normals) = orthogonal_fan_outward_normals(topo, &junction.face_fan) else {
            continue;
        };
        let mut directions = Vec::with_capacity(3);
        for &contour in &[ia, ib] {
            match contour_direction_away_from(topo, &plan.contours[contour], junction.vertex) {
                Ok(Some(direction)) => directions.push(direction),
                _ => continue 'junctions,
            }
        }
        for &edge_id in &junction.unselected_sharp_edges {
            let Some(direction) = topo
                .edge(edge_id)
                .ok()
                .and_then(|edge| edge_direction_away_from(topo, edge, junction.vertex))
            else {
                continue 'junctions;
            };
            directions.push(direction);
        }
        if !trihedral_corner_is_convex(&normals, &directions) {
            continue;
        }
...
/// Outward normals of a three-face planar fan, `None` when any face is not
/// planar or the three planes are not mutually perpendicular.
fn orthogonal_fan_outward_normals(topo: &Topology, faces: &[FaceId]) -> Option<[Vec3; 3]> {
    if faces.len() != 3 {
        return None;
    }
    let mut normals = [Vec3::new(0.0, 0.0, 0.0); 3];
    for (slot, &face) in normals.iter_mut().zip(faces) {
        *slot = topo.face(face).ok()?.effective_plane_normal()?;
    }
    let orthogonal = normals
        .iter()
        .enumerate()
        .all(|(i, n)| normals.iter().skip(i + 1).all(|m| n.dot(*m).abs() <= 1e-10));
    orthogonal.then_some(normals)
}

/// A trihedral corner is convex exactly when every edge leaving the vertex
/// points into the material of every fan face. On a concave (pocket) corner
/// the edges leave along the opposing wall's outward normal instead, and the
/// miter's retained-edge vertex `vertex - n*r` would sit inside the material.
fn trihedral_corner_is_convex(normals: &[Vec3], directions: &[Vec3]) -> bool {
    directions
        .iter()
        .all(|d| normals.iter().all(|n| d.dot(*n) <= 1e-10))
}

/// Whether `edge_id` lies on any wire of `face_id`.
fn edge_in_face(topo: &Topology, face_id: FaceId, edge_id: EdgeId) -> Result<bool, BlendError> {
...
///
/// `FilletPlan` drops tangent-continuous selected edges, so a continuation
/// collinear with the seam can be a requested member of this same build and
/// still be absent from `plan.selected_edges`: the seam is then invisible to
/// the plan's own contours. The material argument survives the drop, because
/// the continuation is still filleted in this same request (or refused on its
/// own terms), so it is still not a hard wall. Measured over the source
/// topology instead of the plan, with the same `1e-6` direction tolerance.
fn requested_collinear_continuation(
    ...
    for &candidate in requested {
        ...
        //
        // Direction by the edge's chord from the shared vertex, not its
        // endpoint tangent: the shape this fallback exists for is a
        // *bowed* fitted boundary, which leaves the seam's far vertex at
        // a steep angle and bows out before returning to the seam line.
        // Read by its endpoint tangent that boundary is a genuine corner
        // and the seam edge is refused; read by its extent the material
        // plainly continues along the seam. Planned contours keep the
        // tangent rule instead, because a contour's spine there is the
        // curve the runout actually lands on.
        ...
        if dir1.cross(direction).length() <= 1e-6 && dir1.dot(direction) >= 1.0 - 1e-6 {
            longest =
                Some(longest.map_or_else(|| delta.length(), |best: f64| best.max(delta.length())));
        }
    }
    Ok(longest)
}

/// How far real material continues past an unselected sharp edge that seams
/// into a different, independently selected contour.
///
...
/// seam whose far vertex is the near terminal of another selected contour
/// running *collinearly* onward, real material plainly continues. The honest
/// measure there is the combined run (the seam stub plus that continuation),
/// which is what this function returns.
///
/// Returns `Ok(Some(continuation_length))` only for that seam shape: a
/// straight unselected edge whose far vertex hosts another non-periodic
/// selected contour, that contour leaving the shared vertex collinearly with
/// the seam edge (by its tangent at the shared vertex, at the same `1e-6`
/// direction tolerance this file already uses elsewhere). A curved
/// continuation that happens to share its chord is a genuine corner, not a
/// seam; so is an angled one.
///
/// The interior-vertex shape (the shared vertex is *not* a terminal of the
/// continuation contour) does not arise on manifold topology, but it is
/// measured rather than assumed: it continues to whichever of that contour's
/// own terminals lies along the seam direction, never more.
fn unselected_edge_seam_continuation(
    ...
    for &contour_index in &other.incident_contours {
        ...
        // Interior vertex: measure to the contour terminal that actually
        // lies along the seam direction instead of claiming an unbounded
        // continuation.
...
/// Radius admissibility for the sharp-mitered two-edge corner, checked on
/// real source-edge lengths **before any topology mutation**.
///
/// The *exact* limit (`r == S`, remainder exactly zero) is admissible: the
/// miter corner closes it without the vanished material, because
...
/// closes each stripe's wire against the crease. The rules refuse a genuine
/// overshoot, never an exact fit.
///
/// Scope: only the equal-radius, orthogonal, convex, planar two-edge corner
/// class the miter construction implements. The qualifying test here (Junction
...
/// mirrors the miter recognizer rather than calling it, so this check can run,
/// and refuse, before that recognizer does any construction work. A junction
/// outside this class (mixed radius, non-trihedral, non-orthogonal, a variable
/// radius law) is left to whatever other path handles it; this function never
/// refuses those, and never falls through to the horn-torus path for a request
/// it does refuse.
///
...
///
/// - a selected contour of length `L` with `m` mitered ends (1 or 2; a contour
///   with 0 qualifying ends is out of scope here): `L >= m*r` (an exact tie is
///   the admissible limit);
/// - the retained (unselected) third edge at each qualifying corner, length
///   `H`: `H >= r`;
/// - the unselected edge(s) at a runout terminal (the far, non-mitered end of
///   a contour whose *other* end is a qualifying corner), each length `L`:
///   `L >= r`. This checks every `unselected_sharp_edges` entry at that
///   terminal, which is never more permissive than checking only the one edge
///   the runout construction ultimately consumes.
///
/// # Errors
/// [`BlendError::RadiusTooLarge`] naming the first violating edge, in
/// deterministic order (junctions and contours in `plan`'s own order, never
/// float-sorted or request-order-dependent), never a generic planning failure,
/// so a caller can recover `max_radius`. Only a genuine overshoot is refused:
/// at the exact limit the rule's length is fully consumed, and
/// `close_collapsed_miter_corner` builds the corner around the material that
/// vanishing consumed.
pub fn check_equal_two_edge_admissibility(
    ...
    for junction in &plan.junctions {
        ...
        for &edge_id in &junction.unselected_sharp_edges {
            // The runout notches only the cap edges lying in the contour's
            // own support faces; another edge at the same vertex keeps its
            // full length and must not refuse the build.
            if !edge_in_face(topo, contour.side1, edge_id)?
                && !edge_in_face(topo, contour.side2, edge_id)?
            {
                continue;
            }
            let edge = topo.edge(edge_id)?;
            let length = vertex_length(edge.start(), edge.end())?;
            // A seam into another selected contour is neither a hard wall nor
            // free material: the honest measure is the stub plus the collinear
            // continuation it seams into. An edge with no such continuation
            // keeps its own unchanged length.
...
///
/// `solid_faces` scopes the "clean trihedral corner" check to the faces of
/// the solid this build actually operates on, which an arena shared across a
/// whole session makes load-bearing (see `incident_face_count`).
...
/// face in `topo` instead of only this solid's own `scope` then counts faces
/// belonging to a *different*, unrelated solid that happens to share the same
/// `VertexId`, silently inflating a corner's apparent valence and making a
/// genuine trihedral corner look non-trihedral on a later, independent
/// operation against the same source. `scope` (the caller's
/// `topo.shell(..).faces()` snapshot, taken before this build's own
/// mutations) is the correct notion of "incident to this corner" and is
/// invariant to unrelated history in the same arena.
...
fn convex_trihedral_setback(
    ...
    // classify a junction as set-back or un-set-back.
    ...
    for &index in indices {
        ...
        // A setback trims the stripe end down to the rolling-ball center, so
        // the center must lie on the bounded stripe centerline. Short-spine
        // junctions (rib tops) are already excluded by the near-equal-stripe
        // guard above, so no separate end-decile limit is needed here.
...
/// Environment-gated trace for the sharp-miter construction, matching this
/// file's existing `BK_CORNER_TRACE` convention.
fn miter_trace() -> bool {
...
/// centre) and [`multi_edge_corner_data`]'s setback classification:
...
/// Classify whether a trihedral junction's terminal sections were
...
fn setback_trimmed_classification(
    ...
    for &index in indices {
        ...
        // A set-back section centre is the solved ball centre itself, so the
        // match band only needs to absorb round-off, scaled to the radius.
        if (section.center - ball_centre).length() <= radius * 1e-6 {
            at_ball_centre += 1;
        }
    }
...
fn collect_terminal_boundaries(
    ...
    for pair in cross_boundaries {
        for handle in [pair.0, pair.1].into_iter().flatten() {
            ...
            // arcs), and an edge merely joining two of those, such as a
            // support boundary piece between two neighbouring terminals, is
            // not part of this closure: registering it here leaves it with an
            // owner-1 slot the built patch never claims, which the
            // post-assembly audit rejects.
...
/// A qualifying sharp-mitered two-edge corner: an equal-radius, orthogonal,
/// convex, planar junction of exactly two selected-edge stripes.
struct MiterCorner {
    vertex: VertexId,
    ...
    ey: Vec3,
    /// Shared top face's outward normal. Either sign of `ex.cross(ey)`
    /// relative to it is admissible (see `detect_equal_two_edge_miter`).
    n: Vec3,
...
/// stripes, `indices`) qualifies for the sharp-mitered two-edge
...
/// mutation; this function performs no refusal itself, so a caller must not
...
fn detect_equal_two_edge_miter(
    ...
    // The splice needs both stripes' registry-built faces; a stripe that
    // went through the contact-edge fallback keeps the ordinary fan.
    if stripe_faces.get(*ia).copied().flatten().is_none()
        || stripe_faces.get(*ib).copied().flatten().is_none()
    {
        return Ok(None);
    }
    let a = &stripes[*ia];
    ...
    let Some(normals) = orthogonal_fan_outward_normals(topo, &faces) else {
        return Ok(None);
    };
    ...
    if sf1b != top_face && sf2b != top_face {
        return Ok(None);
    }
    ...
    let mut directions = vec![ex, ey];
    for &edge_id in &junction.unselected_sharp_edges {
        let Some(direction) = edge_direction_away_from(topo, topo.edge(edge_id)?, junction.vertex)
        else {
            return Ok(None);
        };
        directions.push(direction);
    ...
    // for any unit `u` perpendicular to `n`. Swapping here to force a
    // convention would instead desynchronize `ex`/`ey` from
    // `side_a`/`side_b`/`far_a`/`far_b`.
...
/// two-edge corner.
///
/// The trimmer already delivers exactly the wire the miter needs:
/// `BoundarySplitPolicy::Selective` plus the doubled-back tail drop extend
/// each side support to its own contact and leave the retained vertical edge
/// (incident to `vertex`, direction `-n`) shortened to
/// `p00 = vertex - n*radius`, with no small connector stub between the two.
/// This step therefore adopts the post-trim shape it is handed rather than
/// reconstructing it.
///
...
/// source edge, so both side supports end up on the exact same edge and
/// vertex instances. When the second face arrives with its own coincident
/// copy, that copy is replaced in place and this face's own local
/// `p00`-position vertex is retargeted to the canonical one on whichever
/// other wire edge still references it. That other edge is this build's own
/// side contact, freshly created by the same command's earlier support-face
/// batch trim and never original source topology, so moving its one endpoint
/// in place (same `EdgeId`, same curve) keeps it valid everywhere else it is
/// shared without minting a new edge id the registry's `postassembly_audit`
...
fn adopt_side_face_vertical_edge(
    ...
    for (i, oe) in edges.iter().enumerate() {
        ...
        // retained edge's `S - r` no longer exceeds the radius.
        if !delta
            .normalize()
...
/// corner vertex. `edges` is that face's current outer-wire edge list.
...
/// contacts together directly with no edge between them, which drops the
/// rounded corner-fill arc `mapped_planar_corner_geometry` places there for
/// the unrestricted construction. Mutates `miter.top_face` in place.
///
...
///
/// A contact whose *remaining* length is exactly the radius being consumed
/// has nothing left to shorten, and trimming it produces a
/// single-control-point NURBS the curve builder rejects. That is the
...
fn shorten_top_contacts_and_splice_top_face(
    ...
    // freshly created by this same command's earlier support-face batch
    // trim, never part of the original source solid, so shortening them *in
    // place* (same `EdgeId`, only its curve and one endpoint change) keeps
    // every registry entry already recorded for them (owners: the top face
    // and this stripe's own face) valid: no new edge is minted that the
    // registry's `postassembly_audit` would then need, and be unable, to
    // account for.
    ...
    let mut shorten = |topo: &mut Topology, pos: usize| -> Result<(), BlendError> {
        ...
        // than the remaining length the request does not fit this contact at
        // all: a genuine overshoot. At `length == radius` (within `TOL`) the
        // trim would consume the whole remaining contact, so the contact
        // collapses instead of being trimmed to a degenerate curve.
...
/// corner at `r = S/2`), so the corner that runs second must reuse the
/// vertex the first corner's crease already ends at. Minting a fresh twin
/// per corner would leave the stripe's wire chained through
/// distinct-but-coincident ids, and `insert_miter_crease_into_stripe_face`
/// identifies the break it fills by vertex identity alone (the crease's own
/// endpoints), so the collapsed contact's trim point and the crease's far
/// end must be the same vertex, not merely the same position.
///
/// Below the limit every corner's `vtop` is a different point
/// (`(S - r, r, S)` versus `(r, r, S)` and so on; they coincide only at
/// `r = S/2`), so no existing vertex is ever within `TOL` and a fresh one
/// is always minted.
...
/// into the miter's meeting vertex (see
/// [`shorten_top_contacts_and_splice_top_face`]).
///
/// The contact is dropped from whichever of the corner's two stripe faces
/// owns it, including its pcurve on that face, which described a boundary the
/// face no longer has, and restricted in place to `vtop_vertex` with
/// `EdgeCurve::Line` (a zero-length segment, whose geometry *is* its
/// vertices). It stays in the shared face's wire: that face's whole boundary
/// is then a point, and `fillet_builder.rs`'s zero-extent exclusion drops it
/// from the result shell. Its registry entry is retired so its planned
/// two-owner use (the shared face, excluded, and the stripe face, no longer
/// bounded by it) is no longer audited.
...
/// for a miter vertex, so the wire is open at every end this corner owns:
/// exactly one break for a stripe with one mitered end, two for the
/// four-edge pillow's stripes, whose both ends are mitered and are closed by
/// two *different* corners' creases. The break this call fills is therefore
/// identified by the crease itself (the one open position whose two sides are
/// exactly the crease's own endpoint vertices) and its forward sense follows
/// from which of those two vertices the preceding edge leaves off at. Nothing
/// is assumed from the stripe's spine-start/spine-end convention or from which
/// edge happens to sit next to a break. At the exact limit that distinction is
/// load-bearing: the collapsed shared-face contact shares its single vertex
/// with the crease's far end, so a contact-position rule would insert the
/// crease on the wrong side of the gap and leave the wire open.
...
/// sharp-mitered two-edge corner, from `p00_vertex` to `vtop_vertex`.
...
/// Close the sharp-mitered two-edge corner when the material it closes
...
///   trimmer leaves along their own bottom edges and there is no vertical
///   edge for `adopt_side_face_vertical_edge` to adopt: they meet at the
///   source vertex at `p00` alone;
...
///   case does would trim it to a point;
...
/// face) no longer has two live consumers to audit. The rounded corner-fill
/// arc and the shared face itself are left exactly as the trimmer built
/// them; the pipeline's own zero-area handling excludes both from the
/// published shell, and one exact crease edge closes each stripe's wire
/// against its own side contact.
///
/// Returns `Ok(false)` when the collapse is *not* the reason the retained
/// vertical edge is missing, so the caller can report the attempt's own
/// error unchanged.
...
/// Build the sharp-mitered two-edge corner in full: the retained vertical
/// edge restricted to `[0,h]` and shared by both side supports, each
/// stripe's shared-face contact shortened to the top-contact meeting vertex
/// with the top face's rounded corner-fill arc removed, and one exact
/// `Ellipse3D` crease edge shared by both stripe faces installed in place of
/// their deferred cross-section placeholder. No corner face is created: that
/// is the whole per-corner obligation. When the retained third edge and both
/// contacts have collapsed at the exact limit,
/// [`close_collapsed_miter_corner`] closes the same corner with the same
/// crease instead.
fn apply_equal_two_edge_miter_corner(
    ...
    // limit there is no such edge left to adopt, and the corner closes on
    // the two stripe faces instead.
    ...
    // collapses into the meeting vertex instead.
    let vtop_vertex =
...
pub fn compute_ordered_corners(
    ...
    // The sharp-mitered two-edge corner builds no corner face: the crease
    // edge and the two new vertices are its entire contribution, so its
    // vertices are tracked here and skipped by the junction-fan pass below.
    ...
    for junction in junctions {
        ...
        if let Some(miter) = detect_equal_two_edge_miter(
            topo,
            junction,
            stripes,
            &indices,
...
mod tests {
    ...
    /// The un-set-back trihedral junction (the 90:60:10 corner the
    /// near-equal-stripe guard deliberately leaves untrimmed) classifies
    /// `Some(false)` and builds its cap on the geometrically determined
    /// `r*sqrt(2)` sphere through contacts at exactly `r` from the vertex.
    /// The build-level half of this regression is
    /// `regress_fillet_unset_back_trihedral_corner` in `crates/operations`.
    #[test]
    fn unset_back_junction_classifies_false_and_cap_is_r_sqrt2() {
    ...
    #[test]
    fn setback_junction_classifies_true_with_bit_exact_contacts() {
        let radius = 0.5;
    ...
    /// A synthetic mixed junction (two stripes' terminal sections trimmed to
    /// the ball centre, one left untrimmed) is a genuine inconsistency and
    /// must be refused.
    #[test]
    fn mixed_trimmed_junction_is_refused() {
        let radius = 0.5;
        let mut topo = Topology::new();
        let solid = make_box(&mut topo, Vec3::new(30.0, 30.0, 30.0));
        let (vertex, selected) = box_edges_at_origin(&topo, solid);
    ...
    /// The exact `Ellipse3D` frame that carries the sharp-mitered two-edge
    /// crease.
    ///
    ...
    /// (see `brepkit_math::frame::Frame3`). Passing the plane normal as
    /// `(ey-ex)` or `(ex-ey)` is a 50/50 guess at which one lands `v_axis`
    /// on `+n` (the correct sense) versus `-n` (a crease that dips the wrong
    /// way and fails tangency). Deriving the ellipse's own plane normal as
    /// `u.cross(n)` fixes that choice, because the vector triple product
    /// `(u x n) x u = n` (since `u` and `n` are orthonormal) lands `v_axis`
    /// on exactly `n`. Checked numerically, not just algebraically, and
    /// under a non-axis-aligned rigid placement so the check does not depend
    /// on world-axis alignment.
    ///
    ...
    /// closed-form crease `C(t)=(r(1-cos t), r(1-cos t), h+r sin t)`; (3)
    /// every sampled point lies at exact perpendicular distance `r` from
    /// both stripe cylinder axes (the two full-cylinder implicit equations),
    /// independently of the closed-form match.
    #[test]
    fn miter_crease_ellipse_frame_matches_analytic_reference() {
        ...
        // A non-axis-aligned rigid placement (rotate about Z, then Y, then
        // translate).
        let transform =
        ...
        for i in 0..=steps {
            ...
            // The two stripes' outward normals along the crease are
            // (-cos t, 0, sin t) and (0, -cos t, sin t) in the local frame,
            // so their dot is sin^2(t), in [0,1).
...
/// The seam predicate that decides how much real material continues past an
/// unselected sharp edge.
#[cfg(test)]
mod seam_predicate_tests {
    #![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic)]
    ...
    /// An unselected edge whose far vertex is an *interior* vertex of another
    /// selected contour is measured to the contour terminal that actually
    /// lies along the seam direction, and a non-collinear interior
    /// continuation is not a seam at all.
    #[test]
    fn seam_interior_vertex_branch_is_measured() {
    ...
    /// The collinearity test uses the continuation contour's tangent at the
    /// shared vertex. This circle arc leaves `far` exactly along the seam
    /// edge's own direction and then curves away, so its end-to-end chord is
    /// *not* collinear with the seam edge while its tangent is: judged by the
    /// chord the seam is missed and the short edge is measured alone.
    #[test]
    fn seam_collinearity_uses_the_continuation_tangent() {
        let mut topo = Topology::new();
        let start = vertex(&mut topo, 0.0, 0.0, 0.0);
        let far = vertex(&mut topo, 1.0, 0.0, 0.0);
        ...
        // The chord from `far` to `arc_end` is about (0.924, -0.383, 0), some
        // 22.5 degrees off the seam direction, so chord-based collinearity
        // would reject it.
